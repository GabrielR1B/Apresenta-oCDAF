...
class MultiGauss:
    ...
    def pdf(self, X):
        return np.nan_to_num(
            stats.multivariate_normal.pdf(X, mean=self.mean, cov=self.cov)
        )
...
class Gauss:
    ...
    def pdf(self, X):
        return np.nan_to_num(stats.norm.pdf(X, loc=self.mean, scale=self.std))
...
class MixtureModel:
    ...
    def fit(self, X, weights=None, verbose=False):
        ...
        responsibilities = self.distribution.init_responsibilities(
            X, weights, self.n_components
        )
...
def ilp_select_models_max(models, max_components, verbose=False):
    ...
    try:
        ...
        try:
            # Tentativa com CLARABEL (geralmente uma boa primeira escolha para SOCPs recentes)
