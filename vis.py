...
def show_component_differences(
    loc_models, dir_models, vec_p1, vec_p2, name1, name2, save=True
):
        ...
        for i, gauss in enumerate(loc_model.submodels):
            ...
            if (am_subclusters == 1).all():
                add_ellips(
                    ...
                    gauss.mean,
                    gauss.cov,
                    color=cmap(norm(difference.loc[f"{loc_model.name}_{i}_0"])),
                    alpha=1,
                )
                    ...
                    if f"{loc_model.name}_{i}" == dir_model.name:
                        ...
                        for j, vonmises in enumerate(dir_model.submodels):
                            ...
                            add_arrow(
                                ...
                                10 * dy,
                                fc=cmap(
                                    norm(difference.loc[f"{loc_model.name}_{i}_{j}"])
                                ),
                                arrowsize=4.5,
                                linewidth=1,
                            )
